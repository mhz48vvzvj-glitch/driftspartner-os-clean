...
function financeBudgetChart(lines,projects){
  ...
  const rows=(lines||[]).map((l,idx)=>{
    const base=String(l.category||l.label||'Budsjett').trim()||'Budsjett';
    counts[base]=(counts[base]||0)+1;
    const total=(lines||[]).filter(x=>String(x.category||x.label||'Budsjett').trim()===base).length;
    const note=String(l.notes||'').trim();
    const suffix=total>1?` ${counts[base]}`:'';
    return {id:l.id,label:note&&note.length<22?`${base} - ${note}`:`${base}${suffix}`,budget:Number(l.budget_amount||l.budget||0),actual:Number(l.actual_amount||l.actual||0),type:'budget'};
  });
