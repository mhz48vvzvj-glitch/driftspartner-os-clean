...
async function lookupBrregOrg(orgNo){
  ...
  if(res.status===404)throw new Error('Fant ikke org.nr i Br·nnåysund.');
  if(!res.ok)throw new Error('Kunne ikke hente firmainfo fra Br·nnåysund akkurat nå.');
  return await res.json();
}
async function lookupBrregSupplier(){
  try{
    setFieldText('supLookupOut','Henter firmainfo fra Br·nnåysund...');
    ...
    setFieldText('supLookupOut',`Fant ${entity.navn||'leverandår'} · ${entity.organisasjonsnummer||''}${address?' · '+address:''}`);
  }catch(e){setFieldText('supLookupOut',customerError(e,'Kunne ikke hente firmainfo. Sjekk org.nr og prøv igjen.'))}
}
async function saveSupplier(){try{requireLive('lagre leverandår');if(typeof assertPackageLimit==='function')assertPackageLimit('suppliers','leverandører');if(!supEmail.value.includes('@'))throw new Error('Leverandør må ha e-post.');const r=await db().from('suppliers').insert({name:supName.value,email:supEmail.value,trade:supTrade.value,status:'active'}).select().single();if(r.error)throw r.error;await insertActivity('Leverandør lagret','supplier',r.data.id);await finishAction('Leverandøren er lagret.','market')}catch(e){showDrawer('Leverandør ble ikke lagret',`<div class=\"output\">${esc(customerError(e))}</div>`)}}
function showRfqForm(workOrderId=''){showDrawer('Tilbudsforespårsel',`<input id="rfqWo" type="hidden" value="${esc(workOrderId)}"><label>Tittel</label><input id="rfqTitle"><label>Beskrivelse</label><textarea id="rfqDesc"></textarea><label>Frist</label><input id="rfqDeadline" type="date"><button class="action primary" onclick="saveRfq()">Lagre tilbudsforespårsel</button>`)}
async function saveRfq(){try{if(typeof subscriptionHas==='function'&&!subscriptionHas('rfq'))throw new Error('Tilbudsforespårsel krever Premium.');requireLive('Lagre tilbudsforespårsel');let row={property_id:currentProperty().id,title:rfqTitle.value,description:rfqDesc.value,deadline:rfqDeadline.value||null,status:'Utkast'};if(isUuid(rfqWo.value))row.work_order_id=rfqWo.value;const r=await db().from('quote_requests').insert(row).select().single();if(r.error)throw r.error;await insertActivity('Tilbudsforespårsel opprettet','quote_request',r.data.id);await finishAction('Tilbudsforespårselen er opprettet.','market')}catch(e){showDrawer('Tilbudsforespårsel ble ikke lagret',`<div class=\"output\">${esc(customerError(e))}</div>`)}}
...
function MarketPage(){
  ...
  return `<div class="grid market-page premium-market-page">
    <div class="card s12 module-hero market-hero"><div><small>Marked og tilbud</small><h2>Innkjøp fra behov til valgt leverandår</h2><p>Registrer leverandører, lag tilbudsforespårsel, last opp PDF og vurder pris, forbehold og risiko på valgt eiendom.</p></div><div class="module-actions"><button class="action primary" onclick="showRfqForm()">Lag tilbudsforespårsel</button><button class="action" onclick="showSupplierForm()">Registrer leverandår</button><button class="action" onclick="showOfferForm()">Last opp tilbud</button><button class="action" onclick="showEmailFlow('quote')">Send RFQ e-post</button></div></div>
    <div class="card s12 market-pipeline premium-market-metrics">
      ...
      <div><small>Forespårsler</small><b>${rfqs.length}</b><span>RFQ på eiendommen</span></div>
      <div><small>Sendt/aktiv</small><b>${sent}</b><span>Krever oppf·lging</span></div>
    ...
    <div class="card s4"><div class="dash-title"><h3>Forespårsler</h3><button class="action" onclick="showRfqForm()">Ny</button></div>${rfqCards(rfqs)}</div>
...
function SupplierRegisterPage(suppliers=DP.suppliers||[]){
  return `<div class="grid market-page supplier-only-page">
    <div class="card s12 module-hero market-hero"><div><small>Leverandørregister</small><h2>Leverandører for ${esc(currentProperty()?.name||'valgt eiendom')}</h2><p>Pro-pakken gir leverandårregister med firma, e-post, fagomr·de og historikk. Tilbudsforespårsel, tilbudsopplasting og RFQ er Premium.</p></div><div class="module-actions"><button class="action primary" onclick="showSupplierForm()">Registrer leverandår</button><button class="action" onclick="showEmailFlow('general')">Send e-post</button></div></div>
    <div class="card s4 market-pipeline"><div><small>Leverandører</small><b>${suppliers.length}</b><span>Registrerte firma</span></div></div>
    <div class="card s8"><div class="dash-title"><div><h3>Leverandørregister</h3><p class="muted">Legg inn leverandører med e-post og fagomr·de før Premium RFQ eventuelt tas i bruk.</p></div><button class="action primary" onclick="showSupplierForm()">Ny leverandår</button></div>${supplierCards()}</div>
...
function procurementFlow(rfqs,offers,suppliers){
  const steps=[
    ...
    ['Forespårsel',rfqs.length,'Lag RFQ med frist og beskrivelse',rfqs.length?'ok':'warn'],
    ...
    ['Beslutning',offers.some(o=>/valgt|godkjent/i.test(o.status||''))?1:0,'Velg leverandår og dokumenter valg',offers.some(o=>/valgt|godkjent/i.test(o.status||''))?'ok':'warn']
  ];
...
function supplierTradeList(){return ['Tak og fasade','VVS / rårlegger','Elektro','Brann og sikkerhet','HMS / SHA','Heis','Ventilasjon','Uteomr·de / gr·nt','Snårydding / vinterdrift','Renhold','Bygg / t·mrer','Maler / overflate','L·s / adgang','Skadedyr','Forsikring','Regnskap / Økonomi','Juridisk','Forvaltning','IT / system','Annet']}
...
function AdminPage(){return `<div class="grid admin-page"><div class="card s12">${LaunchControlPage()}</div><div class="card s12 module-hero"><div><small>Kundeoppsett</small><h2>Ny kunde</h2><p>Opprett kunde, eiendom, styre, beboere, leverandører, FDV-mapper, Økonomi og brukere i ·n kontrollert flyt.</p></div><div class="module-actions"><button class="action primary" onclick="showNewCustomerWizard()">Start onboarding</button></div></div><div class="card s6"><h3>Driftskontroll</h3><p class="muted">Sjekker at appen kjårer på ren produksjonspakke og at valgt eiendom er klar.</p><button class="action primary" onclick="runCleanCheck()">Kjør kontroll</button><div id="adminOut" class="output"></div></div><div class="card s6"><h3>Rolle- og tilgangstest</h3>${roleAccessPanel()}</div><div class="card s12"><h3>Aktivitet</h3>${activityCards()}</div></div>`}
...
function launchAdvice(label,ok){
  ...
  const map={
    ...
    'Rolle/tilgang':'Test egne brukere for styreleder, styremedlem, beboer, vaktmester og leverandår.',
    'Avvik':'Opprett minst ett live avvik.',
    'Arbeidsordre':'Lag arbeidsordre fra avvik.',
    'Tilbud/RFQ':'Opprett tilbudsforespårsel og registrer minst ett tilbud.',
    'Dokumentarkiv':'Last opp eller generer minst ett dokument på eiendommen.',
    'Økonomi':'Legg inn konto/reservefond og minst ·n budsjettlinje.',
    'Styre/beboere':'Legg inn styremedlem og beboer/kontaktperson.',
    'Leverandører':'Legg inn leverandår med e-post.',
...
function launchRows(){
  ...
  return [
    ...
    launchStatus('Tilbud/RFQ',rfqs.length>0&&offers.length>0,rfqs.length>0?'Forespårsel klar, mangler tilbud':'Mangler forespårsel/tilbud'),
...
function LaunchControlPage(){
  ...
  return `<div class="dash-title"><div><h3>Lanseringskontroll</h3><p class="muted">Viser om valgt eiendom og produksjonsoppsett er klart for kundepilot og salg.</p></div><div><span class="badge ${type}">${esc(verdict)}</span></div></div><div class="ops-budget-summary"><div><small>Best·tt</small><b>${s.total-s.bad-s.warn}/${s.total}</b></div><div><small>Må fikses</small><b>${s.bad}</b></div><div><small>Må testes</small><b>${s.warn}</b></div></div><div><button class="action primary" onclick="runLaunchControl()">Kjør lanseringskontroll</button><button class="action" onclick="hydrateAll().then(render)">Hent live data</button><button class="action" onclick="location.href='kommersielt.html'">Åpne kommersiell pakke</button></div><div id="launchOut" class="output">${esc(DP.cache.launch_control_result||'Velg live eiendom og kjår kontroll før pilot.')}</div><h4>Statuspunkter</h4>${table(['Omr·de','Status','Neste handling'],launchRows())}`;
}
async function runLaunchControl(){
  ...
  try{
    ...
    if(!isUuid(currentProperty()?.id))throw new Error('Velg en ekte eiendom før du kjårer kontrollen.');
    await hydrateAll();
    const s=launchSummary();
    const lines=[
      `Eiendom: ${currentProperty()?.name||'-'}`,
      `Rolle: ${appRole()||'-'}`,
      `Best·tt: ${s.total-s.bad-s.warn}/${s.total}`,
      `Må fikses: ${s.bad}`,
      `Må testes manuelt: ${s.warn}`,
      s.bad?'Status: Ikke salgsklar ennå. Fiks r·de punkter fårst.':s.warn?'Status: Nesten klar. Test gule punkter live.':'Status: Klar for pilot.'
    ];
    ...
    render();
  }catch(e){setOutputError(out,e,'Lanseringskontroll kunne ikke kjåres.')}
}
function roleAccessPanel(){
  ...
  const expected=[
    ...
    ['leverandår','Marked/tilbud','Kun egne oppdrag/tilbud']
...
function launchChecks(){
  ...
  const checks=[
    ...
    {label:'Rolle/tilgang',ok:!!DP.user?.role&&DP.properties.length>0,group:'Sikkerhet',action:'Test superadmin, styreleder, beboer, vaktmester og leverandår.'},
    ...
    {label:'Styre/beboere',ok:board&&resident,warn:board||resident,group:'Kunde',action:'Legg inn b·de styremedlem og beboer.'},
    {label:'Leverandører',ok:(DP.suppliers||[]).some(s=>String(s.email||'').includes('@')),group:'Innkjøp',action:'Legg inn leverandår med e-post.'},
  ...
  if(hasFinance)checks.splice(checks.findIndex(c=>c.label==='Styre/beboere'),0,{label:'Økonomi',ok:finance.length>0&&budget.length>0,warn:finance.length>0,group:'Økonomi',action:finance.length>0?'Legg inn minst ·n budsjettlinje.':'Legg inn konto/reservefond og budsjett.'});
  if(hasRfq)checks.splice(checks.findIndex(c=>c.label==='Økonomi'||c.label==='Styre/beboere'),0,{label:'Tilbud/RFQ',ok:rfqs.length>0&&offers.length>0,warn:rfqs.length>0,group:'Innkjøp',action:rfqs.length>0?'Registrer minst ett tilbud/PDF.':'Lag tilbudsforespårsel og registrer tilbud.'});
    const completeFlow=devs.length>0&&wos.length>0&&docs.length>0;
  checks.push({label:'Salgsdemo-flyt',ok:completeFlow,warn:devs.length>0&&wos.length>0,group:'Salg',action:'Vis ·n komplett flyt: avvik → arbeidsordre → dokumentasjon → rapport.'});
  if(hasFinance||hasWorkOrders)checks.push({label:'Pro-demo',ok:wos.length>0&&finance.length>0&&budget.length>0,group:'Salg',action:'Pro-demo bår vise arbeidsordre, Økonomi, vedlikeholdsplan og rapport.'});
  if(hasRfq)checks.push({label:'Premium-demo',ok:rfqs.length>0&&offers.length>0,group:'Salg',action:'Premium-demo bår vise Property Brain, RFQ/tilbud og AI-vurdering.'});
...
async function runLaunchControl(){
  const out=document.getElementById('launchOut');
  try{
    if(out)out.textContent='Henter live data og kjårer kontroll...';
    if(!DP.session||!DP.user)throw new Error('Logg inn før lanseringskontroll.');
    if(!isUuid(currentProperty()?.id))throw new Error('Velg en ekte eiendom før du kjårer kontrollen.');
    await hydrateAll();
...
async function seedDemoData(propertyId,planId){
  ...
  try{await safeInsertMany('deviations',[{property_id:propertyId,title:'Tak/VVS - oppf·lging',description:'Demoavvik for visning av saksl·p.',category:'Tak',priority:'H·y',status:'Ny'}])}catch(e){console.warn('Demo avvik hoppet over',e)}
  ...
  if(planId==='premium'){
    try{await safeInsertMany('suppliers',[{name:'Demo Tak AS',email:'tilbud@example.no',trade:'Tak',status:'active'}])}catch(e){console.warn('Demo leverandår hoppet over',e)}
...
function SuperadminOpsPage(){
  ...
  return `<div class="ops-dashboard"><div class="ops-head"><div><small>Intern driftsstatus</small><h3>Helse, bruk og kostnadskontroll</h3><p>Intern oversikt for Driftspartner Nord før dere passerer 50+ kunder. Kundene ser ikke denne siden.</p><p class="ops-runtime">Driftsknapper: direkte klikk aktiv.</p></div><div class="module-actions"><button type="button" class="action primary" onclick="hydrateAll().then(render)">Oppdater live</button><button type="button" class="action" onclick="window.dpOpsAction('showSupportCaseForm'); return false">Ny supportsak</button></div></div><div class="ops-metric-grid">
    ...
    {ok:!quota.limit||aiCalls<quota.limit,warn:true,text:`AI-kall siste 30 dager må overv·kes (${aiCalls}/${quota.limit||0})`},
    {ok:emails<200,warn:true,text:'E-postteller per kunde må følges'},
    {ok:!highUse,warn:true,text:'Varsel ved h·y bruk eller manglende abonnement'}
...
function showSupportCaseForm(){
  showDrawer('Ny supportsak',`<div class="form-grid"><label>Kunde/eiendom<input id="supportCustomer" value="${esc(currentProperty()?.name||'')}"></label><label>Sakstype<select id="supportType"><option>Innlogging</option><option>Dokumenter</option><option>E-post</option><option>AI</option><option>Økonomi</option><option>Annet</option></select></label><label>Alvorlighet<select id="supportSeverity"><option>Normal</option><option>Kritisk</option><option>Lav</option></select></label><label>Status<select id="supportStatus"><option>Ny</option><option>Pågår</option><option>Venter kunde</option><option>L·st</option></select></label><label>Ansvarlig<input id="supportOwner" value="${esc(DP.user?.name||DP.user?.email||'')}"></label><label class="span-2">Intern kommentar<textarea id="supportNote" rows="4" placeholder="Hva må følges opp?"></textarea></label></div><button type="button" class="action primary" onclick="window.dpOpsAction('saveSupportCase'); return false">Lagre supportsak</button><div id="supportOut" class="output"></div>`);
}
...
function backupExportActivityList(){
  ...
  if(!rows.length)return '<div class="empty-state"><strong>Ingen backup- eller eksportrutiner logget.</strong><span>Logg første gjenopprettingstest eller kundeeksport når rutinen er utfårt.</span></div>';
...
function runCostControlCheck(){
  ...
  if(emails>200)warnings.push('H·y e-postbruk');
...
function signatureEmailPayload(row){
  ...
  if(!row)throw new Error('Fant ikke signeringsforespårselen.');
...
async function saveSignatureRequest(relatedType='',relatedId=''){
  ...
  try{
    ...
    await finishAction('Signeringsforespårselen er opprettet.','integrations');
  }catch(e){
    const msg=signatureErrorMessage(e);
...
function integrationItems(){
  ...
  return [
    ...
    {name:'Resend',status:'Aktiv',type:'ok',area:'E-post',detail:'Brukes til demoforespårsler, bestilling, varsler og systemmeldinger.',button:'Åpne e-posttest',action:"location.href='mail-test.html'"},
    ...
    {name:'Br·nnåysundregistrene',status:'Klar for onboarding',type:'ok',area:'Kunde og leverandører',detail:'Org.nr-oppslag kan fylle inn firmanavn og adresse ved opprettelse.',button:canCustomerSetup?'Ny kunde':'Klar',action:canCustomerSetup?'showNewCustomerWizard()':"showIntegrationInfo('Br·nnåysundregistrene')"},
    ...
    {name:'Intern signatur',status:'Aktiv',type:'ok',area:'Kontrakter og vedtak',detail:'Sender signeringsforespårsler på e-post og logger navn, e-post og tidspunkt når noe sendes eller markeres signert.',button:'Ny signering',action:"showSignatureRequestForm('Kontrakt')"},
...
function integrationRoadmap(){
  return `<ol class="integration-roadmap">${[
    ['Br·nnåysund','Org.nr-oppslag i kunde, eiendom og leverandår.'],
    ['Microsoft 365 / Outlook','Måteinnkalling, kalender, e-post og styredokumenter.'],
...
async function testAiIntegration(){
  try{
    ...
    showDrawer('AI-integrasjon',`<div class="info-grid"><section><small>OpenAI</small><strong>${ok?'Klar':'Mangler oppsett'}</strong><span>${data?.has_openai_key?'API-nåkkel er lagt inn':'OPENAI_API_KEY mangler'}</span></section><section><small>Supabase</small><strong>${data?.has_supabase_url&&data?.has_service_role?'Klar':'Mangler oppsett'}</strong><span>${data?.has_supabase_url?'URL er lagt inn':'SUPABASE_URL mangler'} · ${data?.has_service_role?'Service key er lagt inn':'SUPABASE_SERVICE_ROLE_KEY mangler'}</span></section></div><div class="output">${esc(ok?'AI-funksjonen har riktig milj·oppsett. Hvis AI likevel feiler, skyldes det ofte kvote eller betalingsoppsett hos OpenAI.':'Legg inn manglende milj·variabler i Netlify og publiser på nytt.')}</div>`);
  }catch(e){showDrawer('AI-integrasjon',`<div class="output error">${esc(customerError(e,'AI-testen kunne ikke kjåres akkurat nå.'))}</div>`)}
}
function showIntegrationInfo(name){
  const text={
    'Microsoft 365 / Outlook':'Koble måteinnkallinger, styrekalender, e-posttr·der og dokumentdeling. Dette gjår styremodulen mye mer nyttig for kunder.',
    'Intern signatur':'Intern signaturstyring sender forespårsel på e-post og logger hvem som sendte, hvem som signerte og tidspunkt. Dette er enklere, billigere og godt nok for V1 der ekstern signering ikke er et krav.',
    ...
    'Bank / kontoutskrift':'Bankimport bår brukes når Økonomidelen skal vise faktiske transaksjoner, saldo og avvik uten manuell punching.'
  }[name]||'Denne integrasjonen bår beskrives med formål, dataflyt, tilgang og hva kunden før ut av den.';
...
function meetingPayload(){
  ...
  return {to:attendees.map(a=>a.email),subject:title,message,kind:'board',caseId:'meeting',property:currentProperty()?.name||'',property_id:currentProperty()?.id||'',reply_to:replyTo,from_name:fromName,ics:{filename:`${title.replace(/[^\w······-]+/g,'-')}.ics`,content:calendarBase64(icsText)},links,icsText};
}
...
function previewCalendarLinks(){
  try{const payload=meetingPayload();showDrawer('Kalenderlenker',`<div class="stack-list"><section class="mini-record"><div><strong>Outlook</strong><small>Åpne måtet i Outlook-kalender</small></div><button class="action primary" onclick="window.open('${esc(payload.links.outlook)}','_blank')">Åpne</button></section><section class="mini-record"><div><strong>Google Kalender</strong><small>Åpne måtet i Google Kalender</small></div><button class="action" onclick="window.open('${esc(payload.links.google)}','_blank')">Åpne</button></section></div>`)}catch(e){setOutputError(document.getElementById('meetingOut'),e)}
}
async function sendOutlookLightInvite(){
  const out=document.getElementById('meetingOut');
  try{const payload=meetingPayload();if(out)out.textContent='Sender måteinnkalling...';if(location.protocol==='file:'||location.hostname==='localhost'||location.hostname==='127.0.0.1')throw new Error('Måteinnkalling må sendes fra publisert Netlify-side.');const res=await fetch('/.netlify/functions/send-email',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(payload)});const data=await readJsonResponse(res,'E-postfunksjonen svarte ikke riktig. Prøv igjen, eller kontakt Driftspartner Nord hvis feilen fortsetter.');if(!res.ok||!data.ok)throw new Error(data.message||'Måteinnkalling ble ikke sendt.');await insertActivity(`Måteinnkalling sendt: ${payload.subject}`,'meeting',currentProperty()?.id||'-');await finishAction(`Måteinnkalling sendt til ${payload.to.length} mottaker${payload.to.length===1?'':'e'}.`,'integrations')}catch(e){setOutputError(out,e,'Måteinnkalling kunne ikke sendes akkurat nå. Sjekk mottaker og prøv igjen fra live-siden.')}
}

function mailKindLabel(kind){
  return ({general:'Melding',all:'Alle kontakter',deviation:'Avvik',workorder:'Arbeidsordre',quote:'Tilbudsforespårsel',board:'Styresak',resident:'Beboere',manager:'Forvaltere',caretaker:'Vaktmestere',contract:'Kontrakt',purchase:'Bestilling',demo:'Demo'})[kind]||'Melding';
}
...
function mailSubject(kind='general',caseId=''){
  ...
  if(kind==='quote')return `Tilbudsforespårsel - ${name}`;
  if(kind==='board')return `Styresak - ${name}`;
  if(kind==='manager')return `Oppf·lging for forvalter - ${name}`;
...
function mailBody(kind='general',caseId=''){
  ...
  if(kind==='quote')return `${hello}Vi ·nsker tilbud på arbeid knyttet til ${name}${address?', '+address:''}.\n\nSak: ${caseId||'-'}\n\nSend pris, forbehold, leveringstid og relevant dokumentasjon.${sign}`;
...
function emailPayloadFromForm(kind='general',caseId=''){
  ...
  if(!to.length)throw new Error('Velg minst ·n mottaker eller skriv inn en e-postadresse.');
...
async function sendEmailMicrosoft(kind='general',caseId=''){
  ...
  }catch(e){setOutputError(out,e,'Outlook-sending kunne ikke fullfåres. Koble Microsoft 365 og prøv igjen.')}
}
...
function subscriptionPlans(){
  return [
    {id:'start',name:'Start',firstYear:9990,yearTwo:11880,unit:'For mindre sameier og borettslag',fit:'Opptil 20 enheter',items:['1 bygg','Opptil 20 enheter','5 styremedlemmer','20 beboere','5 leverandører','200 dokumenter / 2 GB','300 e-postmottakere per 30 dager','FDV-arkiv','Dokumenth·ndtering','Avviksh·ndtering','Basisanbefalinger','50 AI-klikk per måned','Styreportal','Mobiltilgang']},
...
async function lookupBrregCustomer(){
  try{
    setFieldText('obBrregOut','Henter kundeinformasjon fra Br·nnåysund...');
...
async function lookupBrregOnboardingSupplier(){
  try{
    setFieldText('obSupplierLookupOut','Henter leverandår fra Br·nnåysund...');
    ...
    setFieldText('obSupplierLookupOut',`Fant ${entity.navn||'leverandår'} · ${entity.organisasjonsnummer||''}${address?' · '+address:''}`);
  }catch(e){setFieldText('obSupplierLookupOut',customerError(e,'Kunne ikke hente leverandår. Sjekk org.nr og prøv igjen.'))}
}
...
function validateNewCustomerOnboarding(){
  ...
  if(!hasBoardLead&&!inlineBoardLead)missing.push('Legg til minst ·n styreleder med e-post');
...
function onboardingAdminError(error,step='Onboarding'){
  ...
  if(!msg)return `${step} kunne ikke fullfåres.`;
