...
function showFlowWorkOrderStep(){try{const c=flowSelectedDeviation();if(c.workOrder)return flowDisabledStep('Arbeidsordre finnes allerede','Denne saken har allerede arbeidsordre. Åpne arbeidsordre for å endre status, ansvarlig eller frist.');flowPanel(`<section class="flow-step-card"><small>Steg 1</small><h3>Lag arbeidsordre</h3><p>Opprett en konkret oppgave fra avviket. Fyll inn hva som skal gjøres, og lagre først når innholdet er kontrollert.</p><div class="flow-form-grid"><label>Tittel<input id="flowWoTitle" value="${esc(c.deviation.title||'')}"></label><label>Frist<input id="flowWoDue" type="date"></label></div><label>Beskrivelse<textarea id="flowWoDesc" rows="5">${esc(c.deviation.description||'')}</textarea></label><button class="action primary" onclick="flowCreateWorkOrder()">Opprett arbeidsordre</button></section>`)}catch(e){flowDisabledStep('Velg avvik',e.message)}}
function showFlowRfqStep(){try{const c=flowSelectedDeviation();if(!c.workOrder)return flowDisabledStep('Lag arbeidsordre først','Tilbudsforespørsel må knyttes til en arbeidsordre.');if(c.rfq)return flowDisabledStep('Tilbudsforespørsel finnes allerede','Denne saken har allerede en tilbudsforespørsel. Åpne marked/tilbud for å sende eller endre den.');flowPanel(`<section class="flow-step-card"><small>Steg 2</small><h3>Lag tilbudsforespørsel</h3><p>Beskriv hva leverandørene skal prise. Du kan legge ved PDF og bilder fra tilbudsmodulen etterpå.</p><div class="flow-form-grid"><label>Tittel<input id="flowRfqTitle" value="${esc(c.workOrder.title||c.deviation.title||'')}"></label><label>Frist<input id="flowRfqDeadline" type="date"></label></div><label>Beskrivelse<textarea id="flowRfqDesc" rows="5">${esc(c.workOrder.description||c.deviation.description||'')}</textarea></label><button class="action primary" onclick="flowCreateRfq()">Opprett tilbudsforespørsel</button></section>`)}catch(e){flowDisabledStep('Velg avvik',e.message)}}
function showFlowOfferStep(){flowPanel(`<section class="flow-step-card"><small>Steg 3</small><h3>Last opp tilbud/PDF</h3><p>Registrer leverandørens pris, frist, forbehold og dokumentasjon. Tilbudet knyttes til valgt eiendom.</p><button class="action primary" onclick="showOfferForm()">Åpne tilbudsopplasting</button></section>`)}
function showFlowBoardStep(){try{const c=flowSelectedDeviation();if(!c.offer)return flowDisabledStep('Last opp tilbud først','Styregodkjenning bør bygge på mottatt tilbud og pris.');flowPanel(`<section class="flow-step-card"><small>Steg 4</small><h3>Styregodkjenning</h3><p>Lag et styrepapir som dokumenterer vurdering, beslutning og anbefalt leverandør.</p><label>Notat til styrepapir<textarea id="flowBoardNote" rows="6" placeholder="Skriv kort vurdering, vedtak og eventuelle forbehold."></textarea></label><button class="action primary" onclick="flowBoardApproval()">Lagre styregodkjenning</button></section>`)}catch(e){flowDisabledStep('Velg avvik',e.message)}}
function showFlowContractStep(){try{const c=flowSelectedDeviation();flowPanel(`<section class="flow-step-card"><small>Steg 5</small><h3>Generer kontrakt</h3><p>Opprett kontrakt med signaturfelt. Kontrakten lagres i dokumentarkivet og kan også lages uten tilbud hvis styret ønsker det.</p><label>Kontraktstekst / vilkår<textarea id="flowContractNote" rows="6" placeholder="Skriv leveranse, pris, frist, forbehold, HMS-krav og andre vilkår."></textarea></label><button class="action primary" onclick="flowContract()">Generer kontrakt</button></section>`)}catch(e){flowDisabledStep('Velg avvik',e.message)}}
function showFlowFdvStep(){try{flowSelectedDeviation();flowPanel(`<section class="flow-step-card"><small>Steg 6</small><h3>Oppdater FDV</h3><p>Opprett FDV-notat når arbeidet er utført. Dette lukker avviket og markerer arbeidsordren som utført.</p><label>FDV-notat<textarea id="flowFdvNote" rows="6" placeholder="Skriv hva som er utført, dokumentasjon, kontroller og videre oppfølging."></textarea></label><button class="action primary" onclick="flowFdv()">Lagre FDV og lukk sak</button></section>`)}catch(e){flowDisabledStep('Velg avvik',e.message)}}
function showFlowReportStep(){try{flowSelectedDeviation();flowPanel(`<section class="flow-step-card"><small>Steg 7</small><h3>Sluttrapport / utført arbeid</h3><p>Lag en ryddig rapport for styret med hva som er gjort, vurdering, avvik, anbefaling og neste steg.</p><label>Rapporttekst<textarea id="flowReportText" rows="8" placeholder="Skriv hva som er gjort, vurdering, avvik, anbefaling og neste steg."></textarea></label><button class="action primary" onclick="flowReport()">Lagre rapport</button></section>`)}catch(e){flowDisabledStep('Velg avvik',e.message)}}
