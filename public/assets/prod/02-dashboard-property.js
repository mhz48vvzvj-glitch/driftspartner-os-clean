function DashboardPage(){
  ...
  return `<div class="grid">
    ...
    <div class="card s12"><div class="dash-title"><h3>Neste produksjonssteg</h3><button class="action primary" onclick="openModule('cases')">Start sakslop</button></div>${ProductionFlowMini()}</div>
...
function ProductionFlowMini(){
  ...
  return `<div class="ops-budget-summary">${steps.map(s=>`<div><small>${esc(s[0])}</small><b>${s[1]}</b></div>`).join('')}</div><p class="muted">Tallene kommer fra valgt eiendom. Mangler et steg, ma det opprettes live for flyten kan fullfores.</p>`;
}
function AiDirectorCard(){
  const cached=DP.cache.ai_director_answer||'AI Director leser live data fra valgt eiendom nar du starter analysen.';
  return `<div class="dash-title"><div><h3>AI Director</h3><p class="muted">Analyserer avvik, arbeidsordre, FDV, okonomi, tilbud og aktivitet for valgt eiendom.</p></div><button class="action primary" onclick="runAiDirector()">Analyser valgt eiendom</button></div>
...
async function runAiDirector(mode='prioritering'){
  ...
  try{
    ...
    if(!DP.session?.access_token)throw new Error('Du ma vaere innlogget for a bruke AI Director.');
    ...
    const question=document.getElementById('aiQuestion')?.value.trim()||'Hva bor styret prioritere na?';
    ...
    showDrawer('AI Director kunne ikke kjore',`<div class="output">${esc(msg)}</div>`);
  }
}
