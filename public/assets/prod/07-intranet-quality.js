...
INTERNAL_DOCS.push(
  ...
  {id:'confidentiality',module:'Arbeidsavtaler',title:'Taushetserklæring',version:'1.0',audience:['selger','salgssjef','admin'],requiresAck:true,file:'assets/internal-docs/taushetserklaering.pdf',body:'Taushetserklæring for ansatte og innleide.'},
  ...
  {id:'travel-policy',module:'Økonomi',title:'Reisepolicy',version:'1.0',audience:['selger','salgssjef','admin'],requiresAck:false,file:'assets/internal-docs/reisepolicy.pdf',body:'Regler for reise, transport og dokumentasjon.'},
  {id:'gift-policy',module:'Økonomi',title:'Gave- og representasjonspolicy',version:'1.0',audience:['selger','salgssjef','admin'],requiresAck:true,file:'assets/internal-docs/gave-og-representasjonspolicy.pdf',body:'Regler for gaver, representasjon og habilitet.'},
...
function showInternalDocument(id){
  ...
  if(!d)return;
  showDrawer(d.title,`<div class="internal-doc-detail"><small>${esc(d.module)} · versjon ${esc(d.version)}</small><p>${esc(d.body)}</p><div class="validation-box"><strong>Dokument</strong><span>${d.file?'PDF ligger ved og kan Åpnes direkte.':'Dette dokumentet har ikke PDF vedlagt ennå.'}</span></div><div class="module-actions">${d.file?`<button class="action" onclick="openInternalPdf('${esc(d.file)}')">Åpne PDF</button>`:''}${d.requiresAck?`<button class="action primary" onclick="ackInternalDocument('${esc(d.id)}')">Jeg har lest og forstått</button>`:''}</div><div class="validation-box"><strong>Versjonskontroll</strong><span>Alle endringer skal få nytt versjonsnummer, godkjenner og dato.</span></div></div>`);
}
