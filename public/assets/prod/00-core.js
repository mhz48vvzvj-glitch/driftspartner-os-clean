...
async function readJsonResponse(res,fallback='Tjenesten svarte ikke riktig. Publiser siste pakke og prøv igjen.'){
  ...
  if(!contentType.includes('application/json')){
    ...
    return {ok:false,message:`${fallback} Status: ${res?.status||0}. Svar: ${snippet||'tomt svar'}`,status:res?.status||0,raw:snippet};
  }
  try{return text?JSON.parse(text):{ok:false,message:fallback,status:res?.status||0}}
  catch(e){return {ok:false,message:`${fallback} Status: ${res?.status||0}. Ugyldig JSON.`,status:res?.status||0,raw:text.slice(0,220)}}
}
...
function purchasePanel(plan){
  const prices={Start:'990 kr/mnd',Pro:'1 990 kr/mnd',Premium:'3 990 kr/mnd'};
