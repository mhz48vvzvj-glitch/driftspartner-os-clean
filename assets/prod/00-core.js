...
function packageLimitsForPlan(plan=subscriptionPlanId()){
  ...
  const limits={
    ...
    premium:{buildings:50,units:1000,board:9999,residents:1000,caretakers:25,suppliers:500,documents:10000,documentMb:102400,emails:10000,ai:500,properties:10,rfq:500,projects:100,signatures:100}
  };
...
function aiUsageThisMonth(){
  ...
  return rows.filter(row=>{
    const d=new Date(row.created_at||row.createdAt||0);
    if(!Number.isFinite(d.getTime())||d.getMonth()!==month||d.getFullYear()!==year)return false;
    return /ai|brain|director/i.test(`${row.agent||''} ${row.action||''} ${row.entity_type||''}`);
  }).length;
}
