...
function showSupplierForm(){showDrawer('Ny leverandør',`<label>Firma</label><input id="supName"><label>E-post</label><input id="supEmail"><label>Fagområde</label><input id="supTrade"><button class="action primary" onclick="saveSupplier()">Lagre leverandør</button>`)}
...
function onboardingList(items,type,empty){
  ...
  return items.map((item,i)=>`<section class="mini-record onboarding-item"><div><strong>${esc(item.name)}</strong><small>${esc([item.role||item.unit||item.trade,item.email,item.phone].filter(Boolean).join(' · '))}</small></div><button class="action red" onclick="removeOnboardingItem('${type}',${i})">Fjern</button></section>`).join('');
}
...
function addOnboardingBoard(){
  ...
  ensureOnboardingDraft().board.push({name,role:obVal('obBoardRole')||'Styremedlem',email:obVal('obBoardEmail'),phone:obVal('obBoardPhone')});
  clearOb(['obBoardName','obBoardEmail','obBoardPhone']);renderOnboardingDraftLists();
}
function addOnboardingResident(){
  const name=obVal('obResidentName');if(!name)return onboardingNotice('Fyll inn navn på beboer.');
  ensureOnboardingDraft().residents.push({name,role:'Beboer',unit:obVal('obResidentUnit')||'Beboer',email:obVal('obResidentEmail'),phone:obVal('obResidentPhone')});
  clearOb(['obResidentName','obResidentUnit','obResidentEmail','obResidentPhone']);renderOnboardingDraftLists();
}
function addOnboardingSupplier(){
  ...
  ensureOnboardingDraft().suppliers.push({name,email,trade:obVal('obSupplierTrade')});
  clearOb(['obSupplierName','obSupplierEmail','obSupplierTrade']);renderOnboardingDraftLists();
}
...
async function runNewCustomerOnboarding(){
  ...
  try{
    ...
    const users=draft.users.map(r=>[r.name,r.email,r.role,r.phone,r.password]);let userCount=0;
