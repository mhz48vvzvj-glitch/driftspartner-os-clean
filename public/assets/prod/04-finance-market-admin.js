...
function showDemoUserWizard(){
  ...
  const roleOptions=(typeof canManageSuperadmin==='function'&&canManageSuperadmin())?'<option value="styreleder" selected>Styreleder</option><option value="styremedlem">Styremedlem</option><option value="forvalter">Forvalter</option><option value="beboer">Beboer</option><option value="vaktmester">Vaktmester</option><option value="leverandor">Leverand?r</option>':'<option value="styreleder" selected>Styreleder</option><option value="styremedlem">Styremedlem</option><option value="beboer">Beboer</option><option value="vaktmester">Vaktmester</option><option value="leverandor">Leverand?r</option>';
  showDrawer('Ny demo-bruker',`<div class="form-grid two"><label>Pakke<select id="demoPlan"><option value="start">Start - grunnpakke</option><option value="pro" selected>Pro - operativ drift</option><option value="premium">Premium - AI og tilbud</option></select></label><label>Rolle<select id="demoRole">${roleOptions}</select></label><label>Navn<input id="demoName" value="Demo Styreleder ${stamp}"></label><label>E-post<input id="demoEmail" placeholder="demo@kundedomene.no"></label><label>Telefon<input id="demoPhone" value=""></label><label>Midlertidig passord<input id="demoPassword" value="Demo${stamp}!"></label><label class="span-2">Demo-eiendom<input id="demoPropertyName" value="Demo Borettslag ${stamp}"></label><label class="span-2 check-row"><input id="demoSeedData" type="checkbox" checked> Legg inn enkle eksempeldata for valgt pakke</label></div><div class="validation-box"><strong>Viktig</strong><span>Bruk en e-postadresse du kan motta innloggingsmail p?. Testbrukeren f?r bare tilgang til demo-eiendommen.</span></div><button class="action primary" onclick="createDemoUser()">Opprett demo-bruker</button><div id="createDemoOut" class="output">Klar.</div>`);
}
...
async function testCreateUserFunction(){
  ...
  try{
    ...
    if(res.status===405&&/json|application\/json/i.test(res.headers.get('content-type')||'')){if(out)out.textContent='Bruker-tjenesten er publisert og svarer. Prøv å opprette intern innlogging igjen.';return}
...
function onboardingAdminError(error,step='Onboarding'){
  ...
  if(/row level|rls|policy|permission|not authorized|violates row-level/i.test(msg))return `${step}: brukeren har ikke tilgang til å lagre dette. Logg inn som intern admin, eller kontakt Driftspartner Nord.`;
